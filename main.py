...
def _logo_b64(path: str = "assets/logo_company_3.png") -> str:
    """Retourne le logo encodé en base64 (cherche à plusieurs emplacements)."""
    candidates = [
        Path(path),
        Path.cwd() / path,
        Path.cwd() / "assets" / "logo_company_3.png",
        Path(__file__).resolve().parent / path,
        Path(__file__).resolve().parent / "assets" / "logo_company_3.png",
    ]
    for p in candidates:
        try:
            if p.exists():
...
LOGO_PATH = APP_DIR / "assets" / "logo_company_3.png"

if LOGO_PATH.exists():
    st.sidebar.image(str(LOGO_PATH), use_container_width=True)
else:
    st.sidebar.warning("Logo introuvable (assets/logo_company_3.png)")
